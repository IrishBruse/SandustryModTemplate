...
Import debug from `./debug` in `src/management-button-example/main.ts` (not `@modkit/debug`) so release builds can stub it via `modkit/debug/empty.ts`.
...
```ts
// src/management-button-example/debug/index.ts — debug builds
...
// src/management-button-example/main.ts
