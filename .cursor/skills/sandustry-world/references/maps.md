...
`map.width` / `map.height` in `modinfo.json` are integers from **1** to **3840** (loader `MAX_MAP_DIMENSION`). Campaign / Void saves still allocate **3840 x 3840**. A custom map can be smaller (the integration host uses **80 x 80**).
