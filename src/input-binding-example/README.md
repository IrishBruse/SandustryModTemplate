...
Sandkit bindings work well for letter keys. Use a capture-phase `keydown` listener (see [`overlay-hotkey-example`](../overlay-hotkey-example/)) when the game swallows function keys such as **F7**.

Bindings stay registered for the process lifetime. Hot reload re-runs `main.js` and registers them again.

## Copy this mod

Copy `src/input-binding-example/` to `src/<your-mod>/`. Set binding ids, default keys, and handlers in `main.ts`.
