...
Progress max is **7**. `PI(key, 7)` sets **Starting game**.
...
1. Load save (`electron.load` when the host check in `electron.md` is true; otherwise IndexedDB).
...
8. If packed `shadow.data` length is not `shadowMap.data.length`, full `shadows.refresh`.
  Else `data.set`.
9. `await SI` - **Compiling shaders...** overlay until warmup finishes. `SI` is **null** when `sessionStorage.splashShown` is set **and** the URL has no `db_load`.
