...
| `src/<name>/CHANGELOG.md`      | Optional. Player-facing changelog. `npm run publish` uses the `##` section that matches `modinfo.version` for Steam change notes (`## Unreleased` if that version heading is missing).                                                                                                       |
...
| `scripts/sandustry/workshop-files.js`   | Shared `workshop/` paths plus `CHANGELOG.md` change notes                                                     |
| `scripts/sandustry/publish-workshop.js` | `npm run publish` — release-build to `.tmp/publish/`, SteamCMD upload                                         |
