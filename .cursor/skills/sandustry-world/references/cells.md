...
Use `api.grid.isCellEmptyAtCell`, `isTerrainAtCell`, `elements.getTypeAtCell`, `elements.getMatterTypeAtCell`, `terrains.getTypeAtCell` in mod code.
...
Engine-only `matters.register` / `getMatterTypeFromId`: **sandustry-internals** `references/engine.md`.
