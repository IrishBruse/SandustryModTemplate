...
Import `@modkit/ui/tailwind.css` and insert the string into a `<style>` tag. The build compiles only class names from files esbuild packed into `main.js`. See [examples/overlay-hotkey/main.ts](../../examples/overlay-hotkey/main.ts).
