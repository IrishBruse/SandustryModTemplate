...
Import debug from `./debug` in `src/gif-recorder-example/main.ts` (not `@modkit/debug`) so release builds can stub it via `modkit/debug/empty.ts`.
...
```ts
// src/gif-recorder-example/debug/index.ts — debug builds
...
// src/gif-recorder-example/main.ts
