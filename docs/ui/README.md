...
The build compiles only class names from files esbuild packed into `main.js`. Sandkit does not load a CSS file, so [src/overlay-hotkey/main.ts](../../src/overlay-hotkey/main.ts) still inserts that compiled sheet.
