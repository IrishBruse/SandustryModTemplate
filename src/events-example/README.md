...
2. On first load, look for the toast **Events Example — game ready**.
...
Copy `src/events-example/` to `src/<your-mod>/`. Add listeners with `api.events.on` and pass each unsubscribe function to `onDispose`.
