...
4. Open the worker / DevTools console for `[author.worker-api]` probe lines.
...
Copy `examples/worker-api/` to `src/<your-mod>/`. Extend the probe list in `worker.ts` when you check new API paths. Set `id`, `name`, `author`, and `description` in `mod.ts`.
