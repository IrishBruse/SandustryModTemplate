...
- **Debug / hot reload** — companion mod `src/hot-reload` (DevTools, auto-load last save, F3, local-mod file watch, automatic Sandkit dispose wrap). Free `reloaded` comes from the loader patch. `console` → file log is still an esbuild inject.
