...
Import debug from `./debug` in `src/retro-game-example/main.ts` (not `@modkit/debug`) so release builds can stub it via `modkit/debug/empty.ts`.
...
```ts
// src/retro-game-example/debug/index.ts — debug builds
...
// src/retro-game-example/main.ts
