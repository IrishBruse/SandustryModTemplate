...
Sandustry exposes React on `sandkit.react`. The template does not bundle a separate React copy. TypeScript and esbuild resolve `react` imports to thin shims in `modkit/`.

## Runtime source

`modkit/react.ts` reads the live runtime:
...
At build time, esbuild maps `react` → `modkit/react.ts`.
