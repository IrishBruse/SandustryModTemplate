...
Import debug from `./debug` in `src/overlay-hotkey/main.ts` (not `@modkit/debug`) so release builds can stub it via `modkit/debug/empty.ts`.
...
```ts
// src/overlay-hotkey/debug/index.ts — debug builds
