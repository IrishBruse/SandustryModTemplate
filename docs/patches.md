...
Define patches in that mod's [`mod.ts`](../src/example/mod.ts) with `definePatches`. The build writes `patches.json` into that mod's output folder. The game loader applies those patches to Sandustry JavaScript files (for example `js/bundle.js`).
