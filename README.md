...
- **Watch rebuild** — `npm run dev` writes `main.js`. Restart the game to load it. [Builds](https://ethanconneely.com/SandustryModTemplate/#/builds) · [debug](https://ethanconneely.com/SandustryModTemplate/#/modkit/debug)
- **Debug helpers** — companion mod `src/hot-reload` (F12 DevTools, auto-load save, F3). [Debug](https://ethanconneely.com/SandustryModTemplate/#/modkit/debug)
...
| `npm run dev`       | Watch all src/ mods and write to the OS mods folder (`~/.config/...` or `%APPDATA%`)                                                            |
...
| `npm run publish`   | Runs `npm run build`, then Workshop upload ([SteamCMD](https://developer.valvesoftware.com/wiki/SteamCMD) under `~/.cache/sandustry-steamcmd/`) |
